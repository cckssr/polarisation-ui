...
def _load_points(path: Path) -> dict[str, list[list[float]]]:
    ...
    return {
        gain_id: entry["points"]
        for gain_id, entry in gains_raw.items()
        if entry.get("points")
    }
...
def _print_report(results: list[GainAnalysis], source_name: str) -> None:
    ...
    print(
        col.format(
            "Gain", "Points", "Slope (W/V)", "Offset (W)", "R²", "RMSE (W)", "DR (dB)"
        )
    )
    print(sep)
    for r in results:
        print(
            col.format(
                ...
                (
                    ...
                    if r.dynamic_range_db != float("inf")
                    else "  inf"
                ),
            )
        )
    ...
    if len(results) > 1:
        ref = results[0]
        print("  Cross-gain sensitivity (relative to gain {})".format(ref.gain_id))
        print(sep)
        col2 = "{:>6}  {:>14}  {:>10}  {:>10}"
        ...
        for r in results:
            ...
            print(
                col2.format(r.gain_id, f"{sens:.4e}", f"{ratio:.4f}", f"{ratio_db:.2f}")
            )
        print()
        ...
        for i in range(len(results) - 1):
            ...
            if overlap_lo <= overlap_hi:
                print(
                    f"    Gain {a.gain_id} ↔ Gain {b.gain_id}: overlap {overlap_lo:.3f} – {overlap_hi:.3f} V"
                )
            else:
                gap = overlap_lo - overlap_hi
                print(
                    f"    Gain {a.gain_id} ↔ Gain {b.gain_id}: gap of {gap:.3f} V (no overlap)"
                )
        print()
...
def _plot(
    results: list[GainAnalysis], source_name: str, save_path: Optional[Path]
) -> None:
    n_gains = len(results)
    ...
    fig.suptitle(
        f"Detector calibration — {source_name}", fontsize=13, fontweight="bold"
    )
    ...
    for r, color in zip(results, colors):
        ...
        ax_log.scatter(
            r.voltages, r.powers, s=18, color=color, label=f"G{r.gain_id}", zorder=3
        )
    ...
    for xi, val in zip(x, slope_vals):
        ax_slope.text(
            xi, val * 1.01, f"{val:.2f}", ha="center", va="bottom", fontsize=8
        )
